...
function fireJumpSound() {
  playSound(jumpAudios[jumpIndex]);
  jumpIndex = (jumpIndex + 1) % jumpAudios.length;
}

function fireHitSound() {
  playSound(hitAudios[hitIndex]);
  hitIndex = (hitIndex + 1) % hitAudios.length;
}

function fireScoreSound() {
  playSound(scoreAudios[scoreIndex]);
  scoreIndex = (scoreIndex + 1) % scoreAudios.length;
}
...
const pipes = {
  ...
  update() {
    ...
    for (let i = 0; i < this.list.length; i++) {
      ...
      // സ്കോർ
      if (p.x + this.width < bird.x && !p.cleared) {
        score++;
        p.cleared = true;
...
window.addEventListener('keydown', (e) => {
  if (e.code === 'Space' && gameState !== 'GAMEOVER') handleUserInput();
});

canvas.addEventListener('touchstart', (e) => {
  e.preventDefault();
  if (gameState !== 'GAMEOVER') handleUserInput();
}, { passive: false });

canvas.addEventListener('mousedown', (e) => {
  if (e.button === 0 && gameState !== 'GAMEOVER') handleUserInput();
});
...
function endGame() {
  ...
  if (score > highScore) {
    highScore = score;
    localStorage.setItem('sheikh_best_score', highScore);
  }
...
function toggleTab(type) {
  if (type === 'score') {
    ...
    panelDaily.classList.add('hidden');
  } else {
...
// പേര് എഡിറ്റ് ചെയ്യൽ
btnOpenName.addEventListener('click', (e) => {
  e.stopPropagation();
...
btnSaveName.addEventListener('click', (e) => {
  ...
  if (val) {
    username = val;
    localStorage.setItem('sheikh_username', username);
    dispUsername.textContent = username;
  }
